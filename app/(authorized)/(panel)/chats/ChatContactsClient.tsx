...
export default function ChatContactsClient({ contacts }: { contacts: any }) {
    const [supabase] = useState(() => createClient())
    const [contactsState, setContacts ] = useState<Contact[]>(contacts)
    console.log("🚀 ~ ChatContactsClient ~ contactsState:", contactsState)
    useEffect(() => {
    ...
    return (
        <div className="flex flex-col">
            {contacts && contacts.filter((contact: any) => contact.in_chat === true).map((contact: any) => {
                return <ContactUI key={contact.wa_id} contact={contact} tag = {contact.tags} />
            })}
